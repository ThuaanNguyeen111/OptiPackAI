...
describe('OrderGroupsService — getPackableItemsForGroup (lọc đơn canceled + sự cố logistics)', () => {
  ...
  //!=============================================
  it('product_master TÌM THẤY nhưng dimension bị thiếu/undefined -> throw ORD_GROUP_PACKAGING_PROFILE_NOT_READY (không phải 500, không dùng số đo mặc định — BE-1)', async () => {
    mockOrderFind([
      {
    ...
    productMasterModel.find.mockReturnValue({
      lean: jest.fn().mockResolvedValue([
        // KHÔNG có field dimension — mô phỏng đúng data rác chèn tay
        { seller_sku: 'SKU-RAC', is_fragile: false, packaging_profile_status: 'ready' },
      ]),
    });
