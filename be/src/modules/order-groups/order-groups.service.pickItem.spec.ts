...
describe('OrderGroupsService — pickItem (validate SKU thuộc group trước khi trừ tồn)', () => {
  ...
  function mockGroupHasOnlySku(realSku: string): void {
    ...
    // getPackableItemDetail() -> getPackableItemsForGroup(), hàm này
    // ném ORD_GROUP_PACKAGING_PROFILE_NOT_READY nếu SKU của group chưa
    // có hồ sơ đóng gói kho đã xác nhận. Test này kiểm tra bước validate
    // SKU thuộc group, nên phải mock hồ sơ HỢP LỆ để không bị chặn sớm
    // bởi readiness và che mất hành vi đang cần kiểm chứng.
