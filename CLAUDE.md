...
- **Admin tắt MFA hộ → thông báo user (2026-09-14, KHÔI PHỤC 2026-09-18)**: `adminDisableMfa()` gọi cổng `NotificationsService.notify()` (in-app type `mfa_disabled` + email `mfaDisabledTemplate`). FE hiện popup `MfaDisabledNotice` khi user đăng nhập / đang trong phiên (poll **10s**). Không báo lại nếu MFA vốn đã tắt. Lỗi gửi thông báo không làm fail thao tác tắt MFA.
