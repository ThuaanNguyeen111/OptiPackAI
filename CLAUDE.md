...
- **Admin tắt MFA hộ → thông báo user (2026-09-14, KHÔI PHỤC 2026-09-18)**: `adminDisableMfa()` gọi cổng `NotificationsService.notify()` (in-app type `mfa_disabled` + email `mfaDisabledTemplate`). FE hiện popup `MfaDisabledNotice` khi user đăng nhập / đang trong phiên. **ĐÃ THAY ĐỔI (2026-09-18)**: poll **45s** (không còn 10s) + delay 1.5s sau mount — `ThrottlerGuard` global 20 req/phút, poll 10s cộng `GET /users`/`unread-count` làm trang sau login quay spinner mãi / không có dữ liệu. `apiRequest` timeout **12s** (JwtStrategy đọc Redis trước Mongo; Redis treo thì fetch không settle). Không báo lại nếu MFA vốn đã tắt. Lỗi gửi thông báo không làm fail thao tác tắt MFA.
- **ĐÃ VÁ (2026-09-18) — merge/sync `be/` từ main + rewrite chuông FE đã làm mất nửa luồng MFA**: enum mất `MFA_DISABLED` (chỉ còn comment), `adminDisableMfa()` về bản cũ không `notify()`, `notifications.api.ts` đổi sang `listNotifications`/`markNotificationRead` nhưng `MfaDisabledNotice` vẫn import tên cũ → Vite `MISSING_EXPORT` trắng cả `App.tsx`. **Fix**: khôi phục enum + `notify()` + `sendMfaDisabled`; popup FE dùng API chuông hiện tại (không dùng lại `fetchUnreadNotifications`). Không đụng warehouse. Verify: `npx tsc --noEmit`, `jest` users/mail, `npx vite build`.
...
**ĐÃ VÁ (2026-09-18) — tạo kho xong FE biến mất, gán SKU không chọn được kệ**: (1) `listWarehouses({ is_active: true })` bỏ sót document nếu default `is_active` chưa ghi xuống Mongo + fetch list lúc mount ghi đè kho vừa tạo (race với `listUnassignedSkus` nặng). (2) `generateBinLocations` upsert thiếu `warehouse_id` trong `$setOnInsert` → `toBinResponse().warehouse_id.toString()` crash, dropdown kệ trống. Fix: create ghi `is_active: true`, list không filter active, FE không để fetch cũ ghi đè, sinh kệ luôn kèm `warehouse_id`. **ĐÃ THAY ĐỔI so với bản vá cùng ngày**: mapper `oidString()` dùng `instanceof Types.ObjectId` (không `String(object)`), `isActive` không so sánh boolean `!== false`; tab Gán SKU nhận `binId` controlled từ parent — bấm 1 dòng kệ ở tab Sinh kệ sẽ chuyển sang Gán SKU với kệ đó đã chọn. Tạo lại cùng `warehouse_code` bị 409 `WH_WAREHOUSE_CODE_IN_USE` là ĐÚNG (unique index), không phải kho bị xóa. Range generator = 1 dãy × (rack_to−rack_from+1) × (level_to−level_from+1), mã `{zone}-{aisle}-{rack:02}-{level:02}`, upsert idempotent. **Backfill kệ cũ thiếu `warehouse_id` (cùng ngày, theo yêu cầu user)**: `WarehouseService.backfillMissingBinWarehouseIds()` chạy lúc `onModuleInit` + mỗi lần `listBinLocations`/`generateBinLocations`. Gắn `warehouse_id` từ khu; nếu đã có kệ đủ field cùng `bin_code` thì chuyển `sku_bin_assignments.bin_location_id` sang bản đủ rồi xóa bản mồ côi (không để dropdown hiện 2 dòng `A-03-01-01`). Kệ còn nhưng khu đã mất: **giữ, không xóa, không gắn mù**. Lỗi backfill chỉ log, không chặn list kho/kệ. **ĐÃ THAY ĐỔI (cùng ngày) — restart vẫn trống**: `oidString()` dùng `instanceof Types.ObjectId` sai với `_id` của `.lean()` (BSON ObjectId khác prototype) → API list trả `id: ""` → FE `mapWarehouse` bỏ hết. Sửa: đọc `toHexString()`. List kho không còn `Promise.all` với unassigned — lỗi SKU chưa gán không được phép xóa danh sách kho. **ĐÃ THAY ĐỔI (cùng ngày) — có kho nhưng không thấy khu/kệ**: (1) `listZones({ warehouse_id: string })` có thể không khớp field ObjectId trong Mongo. (2) FE `Promise.all(zones, assignments)` rồi mới `setZones` — 1 API kệ/SKU lỗi thì khu cũng không hiện. Sửa: query bằng `Types.ObjectId`, load khu / kệ / assignment tách nhau, thêm `GET /warehouse/warehouses/:id/bin-locations` lấy mọi kệ của kho. **ĐÃ THAY ĐỔI (cùng ngày) — gán SKU báo `platform must be one of tiktok, lazada, tiki`**: không có (và không thêm) dropdown chọn sàn — `platform` lấy từ catalog. Document `product_master` cũ / upsert trước đây chỉ `$set` dimension nên `platform` có thể thiếu; JSON bỏ field `undefined` → ValidationPipe `@IsEnum` fail với đúng câu tiếng Anh đó. Sửa: `parseMarketplacePlatform()` (thiếu → `lazada`), DTO `@IsOptional` + Transform, `findUnassignedSkus`/`assignSkuToBin` luôn ghi `lazada` nếu thiếu, Product Master `$set.platform`, FE map unassigned + luôn gửi `platform` hợp lệ, form hiện dòng chỉ-đọc “Sàn: lazada (lấy từ catalog)”.
